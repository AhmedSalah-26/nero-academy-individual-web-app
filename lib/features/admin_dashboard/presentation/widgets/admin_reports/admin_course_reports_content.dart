...
/// Admin Course Reports Content
class AdminCourseReportsContent extends StatefulWidget {
  const AdminCourseReportsContent({super.key});

  @override
  State<AdminCourseReportsContent> createState() =>
      _AdminCourseReportsContentState();
}

class _AdminCourseReportsContentState extends State<AdminCourseReportsContent> {
  @override
  void initState() {
    super.initState();
    context.read<AdminReportsCubit>().loadReports(type: ReportType.course);
  }
  ...
  Widget _buildReportsList(
    ...
    // Filter to show only course reports
    final courseReports =
        state.reports.where((r) => r.type == ReportType.course).toList();

    if (isLoading && courseReports.isEmpty) {
